...
def render_markdown_report(report: dict) -> str:
    ...
    architecture = report.get("architecture_summary", {})
    ...
    control_plane = architecture.get("control_plane", {})
    system_services = architecture.get("system_services", {})
    service_lines = [
        f"- {name}: `{details.get('status', 'unknown')}`"
        for name, details in system_services.items()
    ...
    return "\n".join(
        [
            ...
            "",
            "## Architecture Layers",
            f"- Control plane: {' -> '.join(control_plane.get('flow', [])) or 'orchestrator -> router -> leader -> specialist'}",
            f"- Router-selected leaders: {', '.join(control_plane.get('router', {}).get('selected_leaders', [])) or 'none'}",
            f"- Router-selected specialists: {', '.join(control_plane.get('router', {}).get('selected_specialists', [])) or 'none'}",
            *(service_lines or ["- No system services recorded"]),
            "",
            "## Evidence",
